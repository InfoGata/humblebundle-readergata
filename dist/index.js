...
const getProxyUrl = (url)=>{
    ...
    const proxiedUrl = `https://cloudcors.audio-pwa.workers.dev?setRequestHeaders=${JSON.stringify(requestHeaders)}&url=${url}`;
    return proxiedUrl;
};
const getHumbleBundleOrder = async (gamekey)=>{
    const formats = [
        "pdf",
        "epub"
    ];
    const bundleUrl = `https://www.humblebundle.com/api/v1/order/${gamekey}?ajax=true`;
    const orderResponse = await application.isLoggedIn() ? await application.networkRequest(bundleUrl) : await fetch(getProxyUrl(bundleUrl));
    const orderJson = await orderResponse.json();
    const filteredDownloads = orderJson.subproducts.filter((sp)=>sp.downloads.some((d)=>d.platform === "ebook")).map((sp)=>({
            title: sp.human_name,
            images: [
                {
                    url: sp.icon
            ...
            sources: sp.downloads[0].download_struct.map((d)=>({
                    name: d.name,
                    source: d.url.web,
                    type: d.name.toLocaleLowerCase() === "pdf" ? "application/pdf" : "application/epub+zip"
                })).filter((s)=>formats.includes(s.name?.toLocaleLowerCase() || ""))
        }));
    return filteredDownloads;
};
const getOrders = async ()=>{
    const bundleResponse = await application.isLoggedIn() ? await application.networkRequest(orderUrl) : await fetch(getProxyUrl(orderUrl));
    if (bundleResponse.status !== 200) return [];
    const bundles = await bundleResponse.json();
    let promises = [];
    for (const bundle of bundles)promises.push(getHumbleBundleOrder(bundle.gamekey));
...
application.onUiMessage = async (message)=>{
    switch(message.type){
        case "check-login":
            getInfo();
            break;
        case "save":
            if (message.simpleAuth) application.onGetFeed = getFeed;
...
application.onGetPublication = async (request)=>{
...
},{}]},["45FMs","57jqn"], "57jqn", "parcelRequire2bc7")
